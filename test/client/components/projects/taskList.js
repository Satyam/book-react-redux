import { TaskList, mapStateToProps } from 'components/projects/taskList.js';
...
describe('Component: taskList', () => {
  ...
  it('With no editTid it should produce 3 Tasks', () => {
    ...
    expect(taskList.find('Connect(Task)')).to.have.lengthOf(3);
    const editTask = taskList.find('Connect(EditTask)');
  ...
  it('With editTid it should produce 2 Tasks and 1 EditTask', () => {
    ...
    expect(taskList.find('Connect(Task)')).to.have.lengthOf(2);
    const editTask = taskList.find('Connect(EditTask)');
    expect(editTask).to.have.lengthOf(1);
